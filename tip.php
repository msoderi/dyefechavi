...
if(!(isset($_GET["id"]) && is_numeric($_GET["id"]))) { header("Location: http://www.synd.it/synd/"); exit; }
$result = mysqli_query($conn, "SELECT * FROM admin__query_sintax_tips WHERE id = {$_GET["id"]}");
$hits = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM admin__query_sintax_tips"));
if(0 == $hits) { header("Location: http://www.synd.it/synd/"); exit; }
...
<br />
		...
		<div class="tit_bot" <?php if(false !== strpos($_SERVER["HTTP_USER_AGENT"],"MSIE")) echo("style=\"background: url('images/bg_tit_ie.png') left top no-repeat !important; width:570px;\""); ?>>
                  	<h1>Lo sapevi che...</h1>
			<div class="text"><p style="font-weight:bold;"><?=$tip["intro"]?></p><p style="margin-bottom:10px;"><?=$tip["descr"]?></p></div>
           	  </div>
...
<?php
 			...
 			$offset = 0;  if(isset($_GET["id"]) && is_numeric($_GET["id"])) $offset  = round($_GET["id"]);
                        $foffset = 0; if(isset($_GET["id"]) && is_numeric($_GET["id"])) $foffset = round($_GET["id"]); $foffset++;
                        $boffset = 0; if(isset($_GET["id"]) && is_numeric($_GET["id"])) $boffset = round($_GET["id"]); $boffset--;
                        ...
                        ?>
                                <div class="rev_tit_bot" style="margin-top: 20px; display: table; <?php if(false !== strpos($_SERVER["HTTP_USER_AGENT"],"Safari")) { ?>width:590px;<?php } else { ?>width: 570px;<?php } ?> <?php if(false !== strpos($_SERVER["HTTP_USER_AGENT"],"MSIE")) echo("background: url('images/bg_nav_ie.png') left top no-repeat !important;"); ?>"><div style="display:table-row;">
                                        <h1 style="display:table-cell; text-align:left; width:200px; vertical-align:middle;">
                                                <a style="color:#004262;" href="tip.php?id=<?=$boffset?>">&laquo;&nbsp;precedente</a>
                                        </h1>
                                        <h1 style="display:table-cell; text-align:center; vertical-align:middle;"><?=$offset?> / <?=$hits?></h1>
                                        <h1 style="display: table-cell; text-align:right; width:200px; vertical-align:middle;">
                                                <a style="color:#004262;" href="tip.php?id=<?=$foffset?>">successivo&nbsp;&raquo;</a>
                                        </h1>
                                </div></div>
                        <?php
                        }
			...
			else if($foffset <= $hits && $boffset < 1)
                        {
                        ?>
                                <div class="rev_tit_bot" style="margin-top:20px; display: table; <?php if(false !== strpos($_SERVER["HTTP_USER_AGENT"],"Safari")) { ?>width:590px;<?php } else { ?>width: 570px;<?php } ?> <?php if(false !== strpos($_SERVER["HTTP_USER_AGENT"],"MSIE")) echo("background: url('images/bg_nav_ie.png') left top no-repeat !important;"); ?>"><div style="display:table-row;">
                                        <h1 style="display:table-cell; text-align:left; width:200px; vertical-align:middle;">
